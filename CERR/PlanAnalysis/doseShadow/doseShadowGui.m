...
switch upper(varargin{1})
    ...
    case 'REFRESHLEGEND'
        ...
        for i = 1:numStructs
            ...
            structUD.index = i;
            if strcmpi(get(hTransSolid{structNum}(1),'visible'),'on')
                structUD.value = 1;
...
%         numStructs = length(structsInThisScan);
    ...
    case 'SETAXISLIMITS'
        ...
        elseif min(lrRatio) <= min(min(ulRatio),min(llRatio))
            if ulRatio(1) <= ulRatio(2) % x < y
                ...
                dyUL = ulPos(4) / ulPos(3) * dx;
                dyLL = llPos(4) / llPos(3) * dx;
                set(ud.handles.axis.lowerleft,'position',[ulPosX-dx/2 ulPosY-dyUL/2 dx dyUL])
                set(ud.handles.axis.lowerleft,'position',[lrPosX-dx/2 lrPosY-dyLL/2 dx dyLL])
            else
                dy = lrPos(4);
                dxLL = llPos(3) / llPos(4) * dy;
                dxUL = ulPos(3) / ulPos(4) * dy;                
                set(ud.handles.axis.lowerright,'position',[ulPosX-dxUL/2 ulPosY-dy/2 dxUL dy])
                set(ud.handles.axis.lowerleft,'position',[llPosX-dxLL/2 llPosY-dy/2 dxLL dy])                
            end            
        end
    ...
    case 'DRAW'
        ...
        if (getappdata(stateS.handle.doseShadowFig, 'CallbackRun') == 1)
            ...
            %image(colorize(ud.shadow.upperleft, ud.colormap, [0 ud.dMax]), 'parent', ud.handles.axis.upperleft);
        ...
        if (getappdata(stateS.handle.doseShadowFig, 'CallbackRun') == 1)
            if iserror, return, end
            %image(colorize(ud.shadow.lowerright, ud.colormap, [0 ud.dMax]), 'parent', ud.handles.axis.lowerright);
        ...
        if (getappdata(stateS.handle.doseShadowFig, 'CallbackRun') == 1)
            if iserror, return, end
            %image(colorize(ud.shadow.lowerleft, ud.colormap, [0 ud.dMax]), 'parent', ud.handles.axis.lowerleft);
...
partialData = (partialDataM/(max-min)) * (size(c,1) + 0.5);
