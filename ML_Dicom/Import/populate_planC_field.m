                ...
                for doseNum = 1:length(RTDOSE)
                    ...
                    if ~isempty(dvhsequence)
                        ...
                        for i = 1:length(DVH_items)
                            ...
                            doseBinsV = dvhsequence.(['Item_',num2str(i)]).DVHData(1:2:end);
                            binWidthsV = diff(doseBinsV);
                            lastBinWidth = binWidthsV(end);
                            binWidthsV(end+1) = lastBinWidth;                            
                            dataS(dvhsAdded+1).DVHMatrix(1,:) = doseBinsV - binWidthsV/2;
                            dataS(dvhsAdded+1).DVHMatrix(2,:) = dvhsequence.(['Item_',num2str(i)]).DVHData(2:2:end);
                            dataS(dvhsAdded+1).structureName = structureNameC{dvhsequence.(['Item_',num2str(i)]).DVHReferencedROISequence.Item_1.ReferencedROINumber};
